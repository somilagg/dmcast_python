...
def getLocalFromEST_time(EST_time) :
	EST_time = timezone('US/Eastern').localize(EST_time)
...
class general_weather(object) :
	...
	def setDateRange(self,var,start,end):
	...
	def get_date_from_datetime(self, val):
		return val.date()
...
#			print_exception()
		return (sum,values,flgs,dates,var)
...
class general_dm_weather(object) :
	...
	def __init__(self,stn,eDate,sister=None) :
	...
	def setup_time(self,eDate) :
		...
		#eTime = apply(DateTime.Date,(eDate.year,eDate.month,eDate.day,23))
		delta = timedelta(hours=1)
		self.eDate = eDate + delta
		self.sTime = datetime(eDate.year,1,1,0)
...
#				print 'stn_dict does not have key',stn_dict.has_key(requested_var),stn_dict[requested_var].has_key(dkey)
	...
	def get_temperature(self,start,end) :
		...
		if sum==0 :
			...
			(sum,values,flags,dates,var) = self.tmpVar.get_observed(self.tmpVar.tmp,start,end)
			if sum == 0 :
				self.statFlg = 1	
				raise weatherError
			...
			if self.tmpVar == None:
				self.setup_sister_tmp()
			(sSum,sVals,sFlgs,sDates,var) = self.tmpVar.get_observed(self.tmpVar.tmp,start,end)
	...
	def get_precipitation(self,start,end) :
		...
		if sum==0 :
			...
			(sum,values,flags,dates,var)=self.prcpVar.get_observed(self.prcpVar.prcp,start,end)
			if sum==0 :
				self.statFlg = 1
				raise weatherError
			...
			if self.prcpVar == None:
				self.setup_sister_prcp()
			(sSum,sVals,sFlgs,sDates,var)=self.prcpVar.get_observed(self.prcpVar.prcp,start,end)
	...
	def get_rh(self,start,end) :
		...
		if sum==0 :
			...
			(sum,values,flags,dates,var)=self.rhVar.get_observed(self.rhVar.rh,start,end)
			...
			if self.rhVar == None:
				self.setup_sister_rh()
			(sSum,sVals,sFlgs,sDates,var)=self.rhVar.get_observed(self.rhVar.rh,start,end)
	...
	def get_lwet(self,start,end) :
		...
		if sum==0 :
			...
			(sum,values,flags,dates,var)=self.lwetVar.get_observed(self.lwetVar.lwet,start,end)
	...
	def loop_through_time(self) :
		...
		obsTime = datetime(self.sTime.year,self.sTime.month,self.sTime.day,self.sTime.hour)
		delta = timedelta(days=31)
		nexTime = obsTime + delta

		while obsTime < self.eDate :
			
			if nexTime > self.eDate :
				end = datetime(self.eDate.year,self.eDate.month,self.eDate.day,self.eDate.hour)
			else :
				end = datetime(nexTime.year,nexTime.month,nexTime.day,nexTime.hour)
			
			start = datetime(obsTime.year,obsTime.month,obsTime.day,obsTime.hour)
...
#---------------------
		...
		for index in range(len(self.dates)) :
			try :
				...
				print("exception in get_downy_mildew_weather")
				break
...
def get_mildew_weather(stn,eDate,flg=0,sister=None) :
	...
	(statFlg, dates,hours,days,tmp,rh,prcp,lwet,ok) = obj.get_downy_mildew_weather()
...
if __name__ == '__main__' :
	...
	print("reached end of get weather")
